...
const selectUserChecks = (email, cb) => {
  let userChecks = [];
...
const selectUser = (email, cb) => {
  query(`SELECT * FROM users WHERE email = $1`, [email], (qErr, qRes) => {
    if (!qErr && qRes.rows[0]) {
      selectUserChecks(qRes.rows[0].email, (err, checks) => {
        if (!err) {
          const userData = {
...
const updateUserChecks = (email, checkIDsToKeep, cb) => {
  selectUserChecks(email, (err, currentChecks) => {
    if (!err) {
      if (currentChecks && currentChecks instanceof Array && currentChecks.length > 0 && checkIDsToKeep && checkIDsToKeep instanceof Array) {
        currentChecks.forEach(check => {
          if (checkIDsToKeep.indexOf(check) < 0) { //check doesn't exit case
            query(`DELETE FROM user_checks WHERE check_id = $1`, [check], (qErr) => {
              if (!qErr) {
                cb(false);
              } else {
